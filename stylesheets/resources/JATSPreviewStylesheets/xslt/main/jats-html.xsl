...
<!-- ============================================================= -->
  ...
  <xsl:template match="front | front-stub">
    <xsl:variable name="doc-lang" select="//front//journal-title/@xml:lang"/>
        ...
        <xsl:if test="abstract | trans-abstract">
          ...
          <section class="metadata abstract">
            <xsl:for-each select="abstract | trans-abstract">
              <!-- Get language from the abstract or default to the document-->
              <xsl:variable name="this-lang" select="(@xml:lang, $doc-lang)[1]"/>
              <article id="{ if ($this-lang != $doc-lang) then concat(name(), '-', $this-lang) else name() }" class="{name()}">
                <!-- Set attribute lang-->
                <xsl:if test="$this-lang != $doc-lang">
                  <xsl:attribute name="lang" select="$this-lang"/>
                </xsl:if>

                <h4 class="callout-title generated">
